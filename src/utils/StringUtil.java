...
public class StringUtil {
    ...
    public static String getRandomAlphabet(int alphabetLength){
		...
		for(int i=1; i<=alphabetLength; i++){
	        ...
	        sb.append( (char) (str) ); //웹보안진단으로 인한 수정
		}
		*/
        Random ranGen = new Random();
        for(int i=1; i<=alphabetLength; i++){
    ...
    */
    ...
    /**
     public static String[] getPagingNum(String type,String pageNo)
     {
     String line[] = new String[3];

     String linesPerPage = PropertyHandler.getValue("CUST."+type+".LINE", "PAGING");   //page별 라인수를 가져온다.
     int nPageNo = Integer.parseInt(pageNo);
     int nLinesPerPage = Integer.parseInt(linesPerPage);
     int nStart = 0;
     int nEnd = 0;

     nStart = (nPageNo - 1) * nLinesPerPage + 1;
     nEnd = nStart + nLinesPerPage - 1;

     line[0] = Integer.toString(nStart);
     line[1] = Integer.toString(nEnd);
     line[2] = linesPerPage;
     return line;
     }
     public static String getDataBaseCheck()
     {
     String rootDir = PropertyHandler.getValue("ROOT_DIR","SYSTEM");
     String dbPropertyFile = PropertyHandler.getValue("DB.Property", "DATABASE").replace("{ROOT_DIR}", rootDir);
     String keyName = "db.url";

     String value = null;

     try {
     Properties props = new Properties();
     FileInputStream fis = new FileInputStream(dbPropertyFile);
